...
<header class="container d-flex justify-content-between mt-4">
    ...
    <section class="container mt-5">
        ...
        <?php try {
            ...
            $sql = "SELECT COUNT(*) FROM info_producto WHERE id_proveedor='$id'";
            $resultado = $conn->query($sql);
            $eventos = $resultado->fetch_assoc();
            $counter = $eventos['COUNT(*)'];

            $sql2 = "SELECT * FROM info_proveedor WHERE id_proveedor='$id'";
            $resultado2 = $conn->query($sql2);
            $eventos2 = $resultado2->fetch_assoc();
            $nombre = $eventos2['nombre_proveedor'];

            if($counter>0) { ?>
                <div class="pt-4 pl-5 mb-5">
                    <h1>No se pudo eliminar el proveedor porque está en uso.</h1>
                    <h2 class="mt-5 pl-5">Nombre del proveedor: <?php echo $nombre; ?></h2>
                </div>
            <?php }

            else {
                $sql = "DELETE FROM info_proveedor WHERE id_proveedor='$id'";
                $resultado = $conn->query($sql); ?>
