...
<header class="container d-flex justify-content-between mt-4">
    ...
    <section class="container mt-5">
        ...
        <?php try {
			...
			$id = htmlspecialchars($_POST['id_prov']);

            $sql = " SELECT * FROM info_proveedor WHERE id_proveedor=".$id;
				
            $resultado = $conn->query($sql) ?>          
            
            <?php while ($eventos = $resultado->fetch_assoc()) { ?>
                <form class="pl-5 pr-5" action="confirmacion_editar_proveedor.php" method="post">
                    <input type="hidden" name="id_prov" value="<?php echo $id; ?>">
                    <div class="form-group">
                        <label for="nombre_p" class="fz-16">Nombre</label>
                        <input required type="text" value="<?php echo $eventos['nombre_proveedor']; ?>" name="nombre_p" class="form-control">
                    </div>

                    <div class="d-flex d-flex flex-row-reverse mb-5">
                        <button type="submit" class="btn btn-dark btn-lg ">Editar proveedor</button>
                    </div>
