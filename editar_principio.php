...
<header class="container d-flex justify-content-between mt-4">
    ...
    <section class="container mt-5">
        ...
        <h1>Editar principio</h1>

        <?php try {
            require_once('db_connection.php');
				
			$id = htmlspecialchars($_POST['id_prin']);

            $sql = " SELECT * FROM info_principio_activo WHERE id_principio=".$id;
				
            $resultado = $conn->query($sql) ?>          
            
            <?php while ($eventos = $resultado->fetch_assoc()) { ?>
                <form class="pl-5 pr-5" action="confirmacion_editar_principio.php" method="post">
                    <input type="hidden" name="id_prin" value="<?php echo $id; ?>">
                    <div class="form-group">
                        <label for="nombre_prin" class="fz-16">Nombre</label>
                        <input required type="text" value="<?php echo $eventos['nombre_sustancia']; ?>" name="nombre_prin" class="form-control">
                    </div>

                    <div class="d-flex d-flex flex-row-reverse mb-5">
                        <button type="submit" class="btn btn-dark btn-lg ">Editar principio</button>
                    </div>
